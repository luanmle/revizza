...
| VIII. Sync Fidelity & State Separation (NON-NEGOTIABLE) | Pass — nenhum modelo tocado por este plano (`NoteType`, `Note`, o novo `Suggestion.note_type`) carrega ou jamais carregou campo de agendamento/estado de cartão (ease, intervalo, due, histórico de revisão); toda a mudança é escopada a Conteúdo da Nota (tipos de nota, campos, mídia, tags). O caminho de sync (`delta`/`full`/`publish`) segue escrevendo exclusivamente `NoteType`/`Note`; nada nesta feature introduz, lê ou grava estado de agendamento local. T016 (ver `tasks.md` → Foundational) registra essa verificação como tarefa explícita, conforme exigido pela regra de Governança de Planejamento do princípio. |
