...
def _make_note(deck, guid, mod, **kwargs):
...
def test_delta_returns_only_changes_since_mod(auth_client, subscribed_deck):
    now = timezone.now()
    _make_note(subscribed_deck, "old", now - timedelta(days=2))
    _make_note(subscribed_deck, "new", now, anki_deck_path="Sub::Nível 2")
    _make_note(
        subscribed_deck, "gone", now, deleted_at=now
...
def test_delta_without_since_mod_returns_everything(auth_client, subscribed_deck):
    _make_note(subscribed_deck, "a", timezone.now())
...
def test_delta_flags_full_resync_on_structural_change(auth_client, subscribed_deck):
    note_type = subscribed_deck.note_type
